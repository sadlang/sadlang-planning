...
```mermaid
flowchart LR
    INC["239 سطر\n#include sad_ui/...\n(232 اقتباس + 7 أقواس زاويّة)"] -->|"يُحلّ عبر"| BASE["target_include_directories\n(core/include قاعدةً)"]
...
**الدليل (GR-01، قياس حاسم بـAmelia الجولة 4):**
- `#include "sad_ui/..."` (بالاقتباس) = **232** سطرًا، توزيعها: **194** داخل `sad_ui/` نفسه (تنتقل **معه**)، **29** في `interpreter/`، **6** في `tests/`، **3** في `platform/` (مصادر أندرويد). إضافةً **7** بصيغة `#include <sad_ui/...>` (أقواس زاويّة) في `runtime/`. **الإجمالي = 239** مرجعًا لترويسات `sad_ui`. و**صفر** في `compiler/`/`tools/`/`apps/`/`shared/`.
...
- ⇒ **نقل مجلد الجذر لا يغيّر أيّ بادئة تضمين** — لأنّ `core/include/sad_ui/` ينتقل معه، وتبقى البادئة صالحة بمجرّد تحديث مسار `target_include_directories` (يتتبّعه `CMAKE_CURRENT_SOURCE_DIR` تلقائيًّا). يسري هذا على الـ239 جميعًا (بما فيها `runtime/` بالأقواس الزاويّة و`platform/`/`tests/`).
...
> |---|---|---|---|
> | `sad_ui` | `sad_graphics` | **`sad_ui/CMakeLists.txt:172`** ⇒ `features/graphics/CMakeLists.txt` | روابط/حُرّاس خارجيّون: `apps:38`، `libraries.cmake:241,250`، `runtime:78,87`، `tests_comprehensive:220,253,266,273` **+ ~20 استعمالًا داخليًّا للاسم في الملفّ المنقول نفسه** (`:176,204,208,224,225,231,232,256,259,262,270,293-295,328,329,334`) تُحدَّث عند إعادة كتابة `add_library` |
> | `sad_rt_ui` | `sad_graphics_runtime` | `runtime/CMakeLists.txt:79` | `runtime` ذاتها `:80,87,89,91,92` (+ `sad_rt_runtime` INTERFACE `:91`) + تعليقات `:8,74,76` |
> | `sad_ui_bridge` | `sad_graphics_bridge` | `libraries.cmake:242` | `apps:37`، `profiler:67`، `repl:51`، `libraries.cmake:249,258,266,275,280,281,284` + تعليقات `:229,233-235,239,245,247` و`sources.cmake:96,98,388-395` |
>
> كلّها **أسماء أهداف CMake** (روابط/حُرّاس `TARGET`/`OUTPUT_NAME` + تعليقات)، **لا مصدر**. (تصحيح Amelia: «wasm» في تعليقات `libraries.cmake` **وصفٌ لا مستهلِك** — `tools/wasm/sad_wasm.cpp:25-27` يستثني الجسر؛ ولا مستهلِك خارجيّ في `tools/wasm`/`tools/lsp`/`distribution`. الأسماء الجديدة الثلاثة **بلا تصادم** — صفر تعريف قائم.)
>
...
**م-1ب — اتّساق رموز المصدر للجسر (تمسّ المصدر، منفصلة):** سطحها **~7 ملفّات مصدر** (مؤكَّد بـAmelia):
- (أ) الترويسة `interpreter/include/ui/sad_ui_bridge.h` ⇒ `sad_graphics_bridge.h` + حارسها `SAD_INTERPRETER_SAD_UI_BRIDGE_H`.
- (ب) الدالّة `installSadUIBridge()`: **تعريفها** `interpreter/src/ui/ui_module_registration.cpp:108` + **3 مواضع نداء** (`apps/sad-run/main.cpp:139`، `tools/profiler/src/main.cpp:399`، `tools/repl/repl_engine.cpp:103`).
- (ج) **4 ملفّات** `#include "ui/sad_ui_bridge.h"` (sad-run/profiler/repl + `interpreter/src/ui/ui_eval_bridge_impl.h`).
- (د) إشارات الترويسات الخاصّة `interpreter/include/ui/ui_eval_bridge.h:66,70` و`ui_eval_bridge_impl.h:18,39,40`.
- **لا اسم صنف/نطاق يتغيّر** (الدالّة حرّة في `Sad::Interpreter`، لا اسم مشتقّ من `sad_ui`).

**تكسر «صفر تعديل مصدر»** عمدًا (رموز لا مجلد)، فتُعزَل بعد خضرة م-1 ولها بوّابتها: بناء أخضر + `grep -rn 'sad_ui_bridge\|installSadUIBridge'` نظيف. اختياريّة لكنّها لازمة للاتّساق التامّ الذي طلبه القرار.
...
- [ ] **صفر تعديل** على ملفات `.cpp/.h`. تحقّق آليّ: عدّ `#include "sad_ui/` (اقتباس) = **232** و`#include <sad_ui/` (أقواس زاويّة) = **7** قبل النقل وبعده (يُثبت سلامة البادئة عبر كلّ الصيغ والمجلّدات).
...
> 🔍 دُقِّقت بـAmelia (bmad-agent-dev, GR-01) — أربع جولات: (1) الأطروحة (نقل منخفض الخطر، صفر تعديل مصدر) **مؤكَّدة**؛ أُضيف الملفّ الحرِج `platform/android/CMakeLists.txt` ومخاطر م-3. (2) خريطة التسمية: كلّ الأسطر دقيقة، لا مستهلِك خارجيّ مفقود، الأسماء الجديدة بلا تصادم؛ صُحِّح عدّ استعمالات `sad_graphics` الداخليّة (~20) وسطح م-1ب (~7 ملفّات). (3+4) قياس حاسم للتضمينات: **239** مرجعًا (232 اقتباس + 7 أقواس زاويّة)، موزّعة sad_ui/interpreter/tests/platform/runtime — صحّح «223» السابق الذي أغفل tests/platform وruntime الزاويّة.
