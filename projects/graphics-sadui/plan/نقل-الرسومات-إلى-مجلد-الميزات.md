...
>
> ✅ **القرارات محسومة** (انظر §6): الاسم `features/graphics`؛ الهدف يُعاد تسميته `sad_graphics` مع **alias توافق** `sad_ui`؛ `third_party` + `رسومات.ص` **ضمن النطاق الآن**؛ زمن التشغيل ينتقل إلى `features/graphics/runtime/`؛ نقل الشبكة **مؤجَّل**.
...
```
features/graphics/
├── CMakeLists.txt        ← كان sad_ui/CMakeLists.txt (الهدف: sad_graphics + alias sad_ui)
...
| `apps/CMakeLists.txt` | `:37-38` | **لا تغيير** — يربط بالأسماء `sad_ui_bridge`/`sad_ui` لا بالمسار ✅ (يُغطّيه alias التوافق) | 🟢 |
...
> **مبدأ تقليل الخطر:** نعيد تسمية الهدف `sad_ui` ⇒ `sad_graphics` لكن **نضيف alias توافق** `add_library(sad_ui ALIAS sad_graphics)` ⇒ كلّ `target_link_libraries(... sad_ui)` القائم (مثل `apps/CMakeLists.txt:38`) **يبقى عاملًا بلا تعديل**. فالتغيير يقتصر على **المسارات** + **سطر إعادة التسمية + alias**، لا على المستهلكين.
>
...
flowchart TD
    ...
    M1["م-1: نقل sad_ui ⇒ features/graphics\ngit mv + reorganized.cmake + android CMakeLists + 4 ملفات cmake + sad_graphics/alias"]:::m
...
2. في `features/graphics/CMakeLists.txt`: `add_library(sad_graphics STATIC …)` + `add_library(sad_ui ALIAS sad_graphics)` (توافق).
3. تحديث `reorganized.cmake:27-28` (المسار).
4. **تحديث `platform/android/CMakeLists.txt:142-152,273-276`** (مسارات المصدر/التضمين المطلقة) — حرِج.
5. تحديث المسارات المطلقة في `cmake/libraries.cmake` + `cmake/tests_comprehensive.cmake` + تعليقات `cmake/sources.cmake:389` و`interpreter/CMakeLists.txt:25,31`.
6. بناء + تحقّق: الهدفان `sad_graphics`/(alias `sad_ui`) يُبنيان، التضمينات تُحلّ بلا تعديل مصدر، الاختبارات خضراء.
...
| ق-2 | اسم هدف CMake | **`sad_graphics`** + `add_library(sad_ui ALIAS sad_graphics)` | اسم معبّر + صفر كسر للمستهلكين |
...
- [ ] الأهداف `sad_graphics` + alias `sad_ui` + `sad_rt_ui` + `sad_ui_bridge` تُبنى.
