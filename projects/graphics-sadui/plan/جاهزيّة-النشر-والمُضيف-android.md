...
| 6 | توافق `targetSdk` مع حدّ Play الجاري | 🟡 `34` (قد يتطلّب رفعًا حسب التاريخ) | `AndroidManifest.xml:7` | 🟠 |
...
> **التمييز:** البنود 1-6 **تقنيّة في المستودع**؛ 7-8 **تشغيليّة في Play Console** (لا تُحلّ بكود). الحواجب الثلاثة (1/2/3) تمنع أيّ رفع أصلًا.
...
`platform/android/` اليوم = **مُشغِّل تطوير** ينتج APK تصحيح يعمل على جهاز (`adb install`) — **ليس** خطّ نشر متجر. تحويله لجاهزيّة Google Play يتطلّب **ثلاثة حواجب** (توقيع إصدار + AAB + أيقونة) ثمّ تحسينات (تعدّد ABI، موارد، توافق SDK)، وإجراءات Console تشغيليّة. وكلّها **شريحة مُضيف منفصلة** عن نقل `features/` وعن تطوير المكتبة.

---

> ⚠️ محتوى **عامّ** — لا أرقام ماليّة ولا أسرار. راجع [GOVERNANCE.md](../../../GOVERNANCE.md).
> 🔍 دُقِّقت بـAmelia (bmad-agent-dev, GR-01): كلّ الادّعاءات مؤكَّدة من الملفّات؛ صُحِّح أنّ بناء تعدّد ABI موجود (`build_android.ps1`) والناقص حزمُه، وأُبرز أنّ `debug.keystore` **متتبَّع في Git** (تناقض في تنبيه الأمان الأصليّ).
