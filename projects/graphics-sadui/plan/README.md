...
### م-مصانع — سدّ تغطية مصانع المترجم (مكتشَف بالتخطيط)
- **المشكلة**: مصانع `BUILTIN_UI_*` في المترجم تغطّي **7/15** عنصرًا أوّليًّا فقط؛ **5 بلا مصنع**: شبكة، صورة، أيقونة، عرض_تمرير، قائمة_كسولة (الأخيرة بلا رمز؛ المسجَّل `عمود_كسول`/`صف_كسول`).
- **المقترح**: إضافة `BUILTIN_UI_*` + خفض في `builtins_ui.cpp` + نداء وقت تشغيل لكلٍّ. التفصيل في [كتالوج-العناصر-ومصفوفة-الاختبار](./كتالوج-العناصر-ومصفوفة-الاختبار.md).
- **معيار القبول**: `نوع(شبكة())`=«كائن» في المترجم مطابقًا للمفسّر، لكلّ العناصر الخمسة.

### م-أسماء — إصلاح تباعد الأسماء الصامت (مكتشَف بالتخطيط)
- **المشكلة**: نمط أ-2b يتكرّر في **3 عناصر**: المستخدم يكتب `رصة`/`مفتاح`/`نص` لكنّ المصنع يطابق `مكدس`/`مبدل`/`نص_عنصر` ⇒ `رصة(...)`/`مفتاح(...)` تفشل في المترجم.
- **المقترح**: مطابقة مزدوجة في `builtins_ui.cpp` (نظير أ-2b المُثبَت) لقبول الاسم المعياريّ والمرادف.
- **معيار القبول**: `رصة()`/`مفتاح()`/`نص()` تُترجَم وتطابق المفسّر.
